...
switchFacility_module.setFacilityData = function() {
    ...
    for (var i = 0; i < len; i++) {
        var data = {
            "lblFacility": {
                text: userInfo.facilityList[i].facilityId + " - " + userInfo.facilityList[i].facilityName
            },
            "rowIndex": i,
            "imgFacility": {
                isVisible: true
    ...
    for (var i = 0; i < len; i++) {
        if (eventobject.parent.id === "fcFacilities") {
            ...
            if ((switchFacility_module.currentIndex - FACILITY_LOOP_LENGTH + i) === selected_row) {
                ...
                if (isIphone && currentForm == "frmSwitchFacility") frmSwitchFacility.fcerror.setVisibility(false);
                else {
...
switchFacility_module.updateFacility = function() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.ORCHESTRATION_SERVICE_NAME_ACTUAL, _operationName, _headerData, inputData, switchFacility_module.updateFacilitySuccessCallback, switchFacility_module.updateFacilityFailureCallback);
    //CO-346
    //}
};
...
switchFacility_module.updateFacilitySuccessCallback = function(response) {
    ...
    if (response !== null && response.opstatus === 0 && response.statuscode === "200" && response.appList != null) {
        ...
        if (!deepLinkInfo.isDeepLinkFlow && !deepLinkInfo.isRefreshFlow) {
            ...
            if (!isIphone) loginModule.processUserAppList(response);
        }
...
function sync_errorCallBack(response) {
    ...
    if (errorCode == 1011 || errorCode == "SY0000E" || (response.errorInfo != null && response.errorInfo.woundSummary2 != null && response.errorInfo.woundSummary2.errorCode != null && response.errorInfo.woundSummary2.errorCode != "" && response.errorInfo.woundSummary2.errorCode == "SY0000E")) {
        dismissLoadingScreen();
...
var searchFacility_data = function(setFacilityData, processingNeeded) {
    ...
    if (processingNeeded !== null && processingNeeded === true) {
        for (var i = 0; i < setFacilityData.length; i++) {
            var data = {
                "lblFacility": {
                    text: setFacilityData[i].facilityId + " - " + setFacilityData[i].facilityName
                },
                "rowIndex": i,
                "imgFacility": {
                    isVisible: true
