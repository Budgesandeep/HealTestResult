...
tasks_cntrl_offline.getTasksList = function() {
    ...
    var wcs = "where userID =" + JSON.stringify(userInfo.userName) + ///UserName is now UserId as per ERSS, Where clause modified 
...
tasks_cntrl_offline.updateTaskOffline = function(selectedTask) {
    function successCallBack(res) {
        ...
        if (valueToShow === 0) frmViewTasksNew.fcmstheader.lbltaskcount.text = valueToShow;
        else frmViewTasksNew.fcmstheader.lbltaskcount.text = userInfo.taskCount;
    ...
    var compPrimaryKey = {
        ...
        patientUuid: selectedTask[0].patientId
    }; ///Akshay - Fixed the error for completing of the data.
...
tasks_cntrl_offline.getOpenTaskLists = function() {
    ...
    var wcs = "where userID =" + JSON.stringify(userInfo.userName) + ///UserName is now UserId as per ERSS, Where clause modified 
        "and facilityId =" + JSON.stringify(userInfo.selectedFacilityId) + "and completedDate = ''";
