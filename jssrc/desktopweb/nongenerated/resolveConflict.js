...
var resolveConflict_cntrl_offline_getCurrentConflictCount = function() {
    ...
    var wcs = "where facilityId = " + JSON.stringify(userInfo.selectedFacilityId) + " and conflictRaisedByUser IS NOT NULL and conflictRaisedByUser <> 'null' and conflictRaisedByUser <> ''" + "and conflictRaisedByUser_Client IS NOT NULL and conflictRaisedByUser_Client <> 'null' and conflictRaisedByUser_Client <> ''"; //false
...
resolveConflict_cntrl_offline.mapSubStepDetailsByFacilityId = function() {
    ...
    var sql = "SELECT DISTINCT a.patientUuid AS patientId, a.woundNumber, a.stepNumber, a.docEntityId, a.encounterId, b.name AS patientName from healingSubSteps AS a, patientInfo AS b" + " where a.facilityId = " + JSON.stringify(facilityId) + " and a.patientUuid = b.patientId " + " and a.conflictRaisedByUser_Client IS NOT NULL and a.conflictRaisedByUser_Client <> 'null' and a.conflictRaisedByUser_Client <> ''" + " and a.conflictRaisedByUser IS NOT NULL and a.conflictRaisedByUser <> 'null' and a.conflictRaisedByUser <> ''" + " order by CAST(patientUuid as INTEGER), CAST(woundNumber as INTEGER),  stepNumber asc";
...
resolveConflict_cntrl_offline.mapSubStepDetailsByPatientIdAndWoundNoAndStepNo = function(patientId, woundNo, StepNo, patientName) {
    function successCallBack(res) {
        ...
        for (i = 0; i < res.length; i++) {
            ...
            if (tempRecord["record_editable_flag"] == "false" || tempRecord["record_editable_flag"] == false) {
                ...
                tempRecord["btnkeepotherversion"] = {
                    skin: "sknbtnradioSelected",
                    setEnabled: false
                };
                tempRecord["btnkeepmyversion"] = {
    ...
    var wcs = "where patientUuid = " + JSON.stringify(patientId) + " and woundNumber = " + JSON.stringify(woundNo) + " and stepNumber = " + JSON.stringify(StepNo)
        //+ " and conflictRaisedByUser_Client = "+JSON.stringify(userInfo.userName) //still can be commented naresh
        + " and conflictRaisedByUser IS NOT NULL and conflictRaisedByUser <> 'null' and conflictRaisedByUser <> ''" + " and conflictRaisedByUser_Client IS NOT NULL and conflictRaisedByUser_Client <> 'null' and conflictRaisedByUser_Client <> ''" + " order by wndSubDescId asc "
    kony.print(wcs);
    com.healogics.offline.healingSubSteps.find(wcs, successCallBack, errorCallBack);
...
var frmConflictResolve_saveUserSelectedRecords = function(objModified) {
    ...
    var wcs = "where patientUuid = " + JSON.stringify(objModified.patientId) + " and woundNumber = " + JSON.stringify(objModified.woundNumber) + " and stepNumber = " + JSON.stringify(objModified.stepNumber) + " and wndSubDescId = " + JSON.stringify(objModified.wndSubDescId);
...
function frmResolveConflict_saveStepDataWithIntegrationService(param) {
    ...
    if (isIpad && SYNC_CONSTANTS.isAppOffline) {
        ...
        com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_SAVEHEALSTEPS_OFFLINE, {}, inputData, resolveConflict_cntrl_offline.rcsaveHealStepsSuccessCallback, resolveConflict_cntrl_offline.rcsaveHealStepsFailureCallback, flag);
    }
}
