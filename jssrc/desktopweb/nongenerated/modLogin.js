...
loginModule.loginAuthentication1 = function(userName, password) {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_USERLOGIN, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken
    }, inputData, loginModule.userLoginSuccessCallback, loginModule.userLoginFailureCallback, false);
};
/************************************************************************************
...
loginModule.loginAuthentication = function(userName, password) {
    ...
    authClient.login({
        ...
        authClient.getUserAttributes(function(response2) {
...
loginModule.getUserFacilityList = function() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_FACILITYLIST, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken,
    ...
    }, inputData, loginModule.getUserFacilityListSuccessCallback, loginModule.userLoginFailureCallback, false);
...
loginModule.userLoginFailureCallback = function(response) {
    ...
    if (response !== null && response.opstatus != 0 && response.errmsg != null) {
        if (response.opstatus_login != null && response.opstatus_login == 0 && response.responsedesc == GENERAL_CONSTANTS.RESPONSE_UNAUTHORIZED) {
            error_label = ERROR_CONSTANTS.LOGIN_ERROR_MESSAGE;
...
loginModule.userLoginSuccessCallback = function(response) {
    ...
    if (response !== null && ((response.statuscode != null && response.statuscode == 200) || (response.responsecode != null && response.responsecode == 0)) && (response.userName != null && response.userName != "null" && response.userName != "" && response.accessToken != null && response.accessToken != "")) {
        //userInfo.clear(); 
        ...
        if (enabledFacilities !== null && enabledFacilities.length > 0) {
            ...
            if (isIpad || isIphone) {
                ...
                if (!deviceRegisteredForPush) pushNotification_registerDeviceForPushNotifications();
            }
            //CO-346
...
loginModule.processUserAppList = function(response) {
    ...
    function compare(a, b) {
        if (parseInt(a.appid) < parseInt(b.appid)) return -1;
        if (parseInt(a.appid) > parseInt(b.appid)) return 1;
        return 0;
    }
...
loginModule.processSideMenuList = function() {
    ...
    var sideMenuList = ["fcmenuitemone", "fcmenuitemtwo", "fcmenuitemthree", "fcmenufour", //"femenuitemfive", 
        "fcmenusix", "fcmenseven", "fcmenueight", "fcmenunine", "fcmenuten", "fcmenueleven", "fcmenutwelve"
    ];
...
loginModule.logoutUserService = function() {
    ...
    try {
        ...
        com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_LOGOUT, MF_SERVICECONSTANTS.OPERATION_NAME_LOGOUT, {
            "Content-Type": "application/json",
            "Authorization": userInfo.headerAccessToken
        }, inputData, loginModule.userLogoutCallback, loginModule.userLogoutCallback);
    }
};
...
loginModule.epicUser = function() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_USERLOGIN, MF_SERVICECONSTANTS.OPERATION_NAME_AUTHENTICATEBYTOKENEPIC, {
        "Content-Type": "application/json",
        "facilityId": userInfo.presentFacilityId,
        "userId": userInfo.userId,
        "userName": userInfo.userName,
        "deviceType": userInfo.deviceType,
        "deviceId": userInfo.sessionId
    }, inputData, loginModule.userEpicLoginSuccessCallback, loginModule.userEpicLoginFailureCallback);
}
loginModule.userEpicLoginSuccessCallback = function(response) {
    ...
    if (response !== null && ((response.statuscode != null && response.statuscode == 200) || (response.responsecode != null && response.responsecode == 0)) && (response.userName != null && response.userName != "null" && response.userName != "" && response.accessToken != null && response.accessToken != "")) {
        ...
        com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.ORCHESTRATION_SERVICE_NAME_ACTUAL, _operationName, _headerData, inputData, com.healogics.patientSummary.patientSummarySuccessCallback, com.healogics.patientSummary.patientSummaryFailureCallback);
    } else {
        //var error_label = response.responsedesc !="" ? response.responsedesc : ((response.errdesc !=null && response.errdesc!="") ? response.errdesc : response.errorMessage);
        var error_label = (response.errorMessage !== null && response.errorMessage !== "") ? response.errorMessage : ((response.errdesc !== null && response.errdesc !== "") ? response.errdesc : response.responsedesc);
        kony.print("error_label:::::" + error_label);
