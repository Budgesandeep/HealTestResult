...
var setPercentageForLoader = function(x) {
    ...
    if (isIpad) {
        ...
        for (i = 1; i <= SYNC_CONSTANTS.totalScopeCount; i++) {
            if (i <= y) frmSelectFacility["loader" + i].skin = "sknlblloadergrey";
            else frmSelectFacility["loader" + i].skin = "sknlblloaderwhite";
        }
    } else {
        if (x == 1) {
            y = 1;
...
var setPercentageForLoader1 = function(x) {
    ...
    for (i = 1; i <= 10; i++) {
        if (i <= y) popupsyncstatus["loader" + i].skin = "sknlblloadergrey";
        else popupsyncstatus["loader" + i].skin = "sknlblloaderwhite";
    }
};