...
function patientListModule_getAllPatientList1(PATIENTFLAG) {
    ...
    if (isIpad && !isNetworkAvailable()) {
        ...
        com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.ORCHESTRATION_SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_PATIENTLIST, {
            ...
            //"facilityId" : userInfo.presentFacilityId
        }, inputData, patientListModule.patientListSuccessCallback, patientListModule.patientListFailureCallback);
    }
}
...
patientListModule.getFilteredPatientList = function() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_PATIENTLIST, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken,
        "userId": userInfo.userId,
        "facilityId": userInfo.presentFacilityId,
    ...
    if (isIphone) {
        ...
        if (response !== null && response.opstatus === 0 && response.statuscode == "200") {
            ...
            if (patientList !== null && patientList !== undefined && patientList.length > 0) {
                for (var i in patientList) {
                    ...
                    if (msrReviewLabel === "" || msrReviewLabel === null) {
                        tempRecord["lblpatientname"] = {
                            "text": patientList[i]["name"],
                            "top": "15dp"
                        };
                    } else {
                        tempRecord["lblpatientname"] = {
            ...
            if (patientListConstants.filterValue == "" && patientListConstants.sortColumn != "") { //This condtion means sorting is hapening
                ...
                var imageName = "";
                if (patientListConstants.sortOrder == "a") imageName = "headerarrow.png";
                else imageName = "headerarrowup.png";
    ...
    if (userInfo.viewPatientList == "true" || isIphone) {
        ...
        if (isIpad && !isNetworkAvailable()) {
            ...
            com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_PATIENTLIST, {
                ...
                "deviceId": userInfo.sessionId
                    //"facilityId" : userInfo.presentFacilityId
            }, inputData, patientListModule.patientSearchSuccessCallback, patientListModule.patientSearchListFailureCallback, "false");
        }
    }
};
...
patientListModule.patientSearchSuccessCallback = function(response) {
    ...
    if (response !== null && response.opstatus === 0 && response.statuscode == "200") {
        ...
        if (currentForm.id == "frmPatientList" && !isIpad && !isIphone) currentForm.fcsearchdropdown.left = parseInt(currentForm.tbpatientsearch.frame.x) + parseInt(currentForm.fcSearchPatient.frame.x) + parseInt(currentForm.fcdashboardsubheader.frame.x);
...
var frmPatientList_onSearchStart = function() {
    ...
    if (currentform.tbpatientsearch.text.length === 0) {
        currentform.fcsearchdropdown.setVisibility(false);
        currentform.forceLayout();
        if (!isIphone) currentform.imgSearchIcon.src = "search.png";
    } else {
        if (!isIphone) currentform.imgSearchIcon.src = "closepopupsmall.png";
    }
    if (!isIphone) {
...
var frmPatientListiPhone_postShow = function() {
    if (frmPatientList.fcoptionsmenu.isVisible) frmPatientList.fcoptionsmenu.setVisibility(false);
    if (frmPatientList.tbpatientsearch.isVisible) frmPatientList.tbpatientsearch.setVisibility(false);
    if (frmPatientList.fcsearchdropdown.isVisible) frmPatientList.fcsearchdropdown.setVisibility(false);
