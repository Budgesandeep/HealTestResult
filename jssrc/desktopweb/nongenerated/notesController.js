...
notes_cntrl_offline.getNotesList = function(notes_info) {
    ...
    var wcs = "where patientName =" + JSON.stringify(notes_info.patientname) + " and woundNo =" + JSON.stringify(notes_info.woundId) + " and patientUuid =" + JSON.stringify(notes_info.patientId) +
...
notes_cntrl_offline.getCreatedTasksInOffline = function() {
    ...
    var wcs = "where userId =" + JSON.stringify(userInfo.userName) + " and patientUuid =" + JSON.stringify(patientSummaryObjects.patientDemoGraphics.patientId) + " and facilityId =" + JSON.stringify(userInfo.selectedFacilityId) + " and deleted = 0 order by createDate desc";
    kony.print("Where Clause getNotesList : " + wcs);
