...
taskListModule.createTask = function() {
    ...
    }, inputData, taskListModule.createTaskSuccessCallback, taskListModule.TaskListModuleFailureCallback);
};
...
taskListModule.getTaskList = function() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_GETTASKLIST, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken,
        "userId": userInfo.userId,
        "facilityId": userInfo.presentFacilityId,
        ...
        "deviceType": userInfo.deviceType,
        "deviceId": userInfo.sessionId
    }, inputData, taskListModule.getTaskListSuccessCallback, taskListModule.TaskListModuleFailureCallback);
...
taskListModule.processTaskList = function(lTaskStatus) {
    ...
    for (var i in task_info.taskList) {
        ...
        tempRecord = {
            ...
            "btnmarkcomplete": taskRecord.taskStatus == "OPEN" ? {
                isVisible: true,
            ...
            "lbldatecompleted": taskRecord.taskStatus == "COMPLETE" ? {
                isVisible: true,
                text: taskRecord.completedDate
            } : {
                isVisible: false,
                text: "Mark as Complete"
            }, //taskRecord.completedDate
            "lblstatus": taskRecord.taskStatus == "OPEN" ? {
                text: "Open"
            } : {
                text: "Completed"
            }, //=="OPEN"?{skin:"sklblgray100"}:{"skin":"sknlblstatuswhite"},
        };
...
taskListModule.updateTask = function(selectedTask) {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.ORCHESTRATION_SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_UPDATETASK, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken,
        "patientId": selectedTask[0].patientId,
        "facilityId": userInfo.presentFacilityId,
        ...
        "deviceType": userInfo.deviceType,
        "deviceId": userInfo.sessionId
    }, inputData, taskListModule.updateTaskSuccessCallback, taskListModule.TaskListModuleFailureCallback);
};
...
taskListModule.updateTaskSuccessCallback = function(response) {
    ...
    if (response !== null && response.opstatus === 0 && response.statuscode == "200") {
        if (response.taskList !== null && response.taskList.length > 0) {
            ...
            if (sortAndFilterConstants.selectedFilter == "lbldatecompleted") taskListModule.processTaskList(task_info.STATUS_OPEN); //
            else taskListModule.processTaskList(task_info.STATUS_ALL);
            var valueToShow;
            /*if(isIpad){
              tasks_cntrl_offline.getOpenTaskLists();
