...
terms_and_conditions_offline.getTermsAndConditions = function() {
    ...
    var wcs = "where facilityId = " + JSON.stringify(userInfo.selectedFacilityId) + "and userId = " + JSON.stringify(userInfo.userId); ///should check facility and logged in user Id to proceed
...
terms_and_conditions_offline.updateTermsAndConditions = function(acceptedTermsAndConditions) {
    ...
    }; ///Akshay : Removed userName and put userId
    kony.print("compPrimaryKey--------->" + compPrimaryKey);
    kony.print("myObj updateTermsAndConditions--------->" + JSON.stringify(myObj));
    com.healogics.offline.termsAndConditions.updateByPK(compPrimaryKey, myObj, successCallBack, errorCallBack, true);
};