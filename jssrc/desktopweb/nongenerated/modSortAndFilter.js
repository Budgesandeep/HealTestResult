...
function getFilter() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_USERPREFERENCELIST, {
        ...
        "deviceType": "",
        "deviceId": "",
    }, inputData, getUserFilter_SuccessCallback, getUserFilter_FailureCallback);
}
...
function getUserFilter_SuccessCallback(response) {
    ...
    for (var i = 0; i < filterKeys.length; i++) {
        ...
        if (key === "lblappttype" || key === "lblcasemanager" || key === "lblweeks" || key === "lblwounds" || key === "lblnextappt" || key === "lbltransferstatus" || key === "lblpatientmsrstatus" || key === "lblhealper" || key === "lblPrecautions" || key === "lblphysician") {
...
function getUserPreferenceList() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.ORCHESTRATION_SERVICE_NAME_ACTUAL,
        ...
        MF_SERVICECONSTANTS.OPERATION_NAME_FILTERPREFERENCE, {
            ...
            "deviceType": "",
            "deviceId": "",
        }, inputData, patientListModule_filterListSuccessCallback, patientListModule_filterListFailureCallback);
}
...
sortAndFilterModule.filterDataBasedonParameters = function(arrList, arrParameters) {
    ...
    if (appliedFilterParameters.length === 0) {
        if (currentfrm.id == "frmViewTasksNew") task_info.filterData = taskListArr; //task list service data
        else patientListConstants.filterData = PATIENT_LIST_SERVICE_DATA;
    } else {
...
sortAndFilterModule.sortSelectedColumnData = function(arr, selectedSortingType) {
    ...
    if (sortAndFilterConstants.selectedFilter == "lblnextappt") {
        ...
        arr.sort(sortAndFilterModule.sortData(sortAndFilterConstants.selectedFilter, selectedSortingType, function(a) {
...
sortAndFilterModule.sortPatientListNextAppointmentDate = function(arr, selectedSortingType, ignoreBlankInSorting) {
    ...
    arrCancelled.sort(sortAndFilterModule.sortData(sortAndFilterConstants.selectedFilter, selectedSortingType, function(a) {
...
sortAndFilterModule.applySeletedFilter = function(currentFormName) {
    ...
    if (popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeys == undefined || popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeys.indexOf("0") === undefined || popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeyValues === null || popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeys.length === 0) {
...
sortAndFilterModule.sortData = function(field, reverse, primer) {
    var key = primer ? function(x) {
        if (typeof x[field] === "object") {
            return primer(x[field]["text"])
        } else {
            return primer(x[field])
        }
    } : function(x) {
        if (typeof x[field] === "object") return x[field]["text"]
        else return x[field]
    };
    reverse = !reverse ? 1 : -1;
    // return function (a, b) {
...
function popupFilter_checkBox_onSelection(eventobject) {
    ...
    if (popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeys != null && ((popupsortandfilter.CheckBoxGroupFilterPopUp.masterData.length - 1 == popupsortandfilter.CheckBoxGroupFilterPopUp.selectedKeys.length) || (eventobject.selectedKeyValues[0][0] == 0))) { //If Select All is checked then select All option OR  If All values are selected except Select All option than also check Select All option
        var arrAllKeys = [];
...
function createFilter() {
    ...
    com.healogics.mfutility.invokeIntegrationService(MF_SERVICECONSTANTS.SERVICE_NAME_ACTUAL, MF_SERVICECONSTANTS.OPERATION_NAME_USERPREFERENCE, {
        "Content-Type": "application/json",
        "Authorization": userInfo.headerAccessToken,
        "userName": "",
        "deviceType": "",
        "deviceId": "",
        "facilityId": userInfo.selectedFacilityId,
    }, inputData, patientListModule_createfilterListSuccessCallback, patientListModule_createfilterListFailureCallback);
}
