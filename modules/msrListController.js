...
msrList_cntrl_offline.getAllMsrListForFacilityID = function(facilityID,callBack){
  ...
  if(patientListConstants.myPatients=="Y"){
    wcs = "where (physicianUserId = "+ JSON.stringify(userInfo.userId) +
      " or caseManagerUserId = " + JSON.stringify(userInfo.userId) +" ) and facilityId = "+ JSON.stringify(facilityID) +" ORDER BY name";
...
msrList_cntrl_offline.getPatientCountForTotalCount = function(callBackForCount,facilityID){
  var wcs = "where (physicianUserId = "+ JSON.stringify(userInfo.userId) +
      " or caseManagerUserId = " + JSON.stringify(userInfo.userId) +" ) and facilityId = "+ JSON.stringify(facilityID)+" and msrFlag = 'true'" +" ORDER BY name";
