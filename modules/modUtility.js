...
com.healogics.utility = (function(){
  ...
  var _formatText = function(widgetID, formatSymbol, stepValue){
    ...
    if (!com.healogics.utility.isEmpty(inputText) &&
        !com.healogics.utility.isEmpty(inputText) &&
        (formatedText.length < inputText.length)) {
  ...
  var _getFullDateTimeFormatEST = function (ESTDate){
    ...
    return (days[x.getDay()] + ", " + (month)  + "/" + x.getDate() + 
            "/" + x.getFullYear() + ", " + hours + ":" + minutes + " " + am_pm);
  };
...
function getCurrentDateTime(){
  ...
  var datestring = ("0" + d.getDate()).slice(-2) + "-" + ("0"+(d.getMonth()+1)).slice(-2) + "-" +
      d.getFullYear() + " " + ("0" + (d.getHours() > 12 ? d.getHours()-12 : d.getHours())).slice(-2) + ":" + ("0" + d.getMinutes()).slice(-2)+" "+(d.getHours() > 12? "PM":"AM");
...
var prptobj={padding:"pkcs5",mode:"cbc",initializationvector:
"1234567890123456"};
...
function showLastSync(){
  ...
  var toShowDate = null, finalSyncTime = null, syncTimeVar = null; 
