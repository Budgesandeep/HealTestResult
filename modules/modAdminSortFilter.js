...
function frmAdministration_Shorting(eventobject){
  ...
  if(popupAdminSortFilter.hboxascending.skin == "skhboxbluebgSelected" || popupAdminSortFilter.hboxdescending.skin == "skhboxbluebgSelected")
  {
    ...
    admin_data.sort(sort_by(sortAndFilterConstants.selectedFilter, sortAndFilterConstants.adminSelectedSortingType, function(a){return a.toUpperCase()}));
...
function popupFilter_checkBox_onSelection1(eventobject){
  ...
  var index = arrSelectedKeys.indexOf("0");
  if (index > -1 || (popupAdminSortFilter.CheckBoxGroupFilterPopUp.masterData.length-1 == 
                     popupAdminSortFilter.CheckBoxGroupFilterPopUp.selectedKeys.length 
...
function frmAdministration_FilterPopup_Clear_Apply(eventobject){
  ...
  if(eventobject.id== "btnfilterclear"){//Clear selected value in selected Filter
    ...
    if(popupAdminSortFilter.CheckBoxGroupFilterPopUp.selectedKeys == undefined || popupAdminSortFilter.CheckBoxGroupFilterPopUp.selectedKeys.indexOf("0") === undefined || popupAdminSortFilter.CheckBoxGroupFilterPopUp.selectedKeyValues===null ||
       popupAdminSortFilter.CheckBoxGroupFilterPopUp.selectedKeys.length===0){
    ...
    {
      kony.print("admin_data--->>"+JSON.stringify(admin_data));
      FACILITY_LIST_SERVICE_DATA.sort(sort_by(sortAndFilterConstants.selectedFilter, sortAndFilterConstants.adminSelectedSortingType, function(a){return a.toUpperCase()}));
      kony.print("sortedList_data--->>"+admin_data);
      glbStartingRecord=0;
      glbEndingRecord=40;
